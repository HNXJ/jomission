"""SCI-WS-OSC-1 arm driver: does the burst mode require local E->E recurrence?

Forked from scripts/whole_system_causal_arm.py, a sealed artifact of WS-PROP-1 (blob
06f4a2a5, sealed_in 2a2f4d8), rather than edited in place.

Additive changes, one principal delta:

  --ee-cut      set every within-area E->E weight to exactly 0.0, after enforcement and
                after EQUAL_G realization, before compilation. Nothing else is touched:
                E->I, I->E, I->I, the retinal interface and the long-range edges keep
                their inherited values. Realization is measured, not assumed.

The periodicity estimator's window, maximum lag and surrogate count are read from the
spec's "oscillation" block, so they are sealed inputs rather than constants chosen here.

Both arms run at g = 0, the fully disconnected condition WS-AUTH-2 sealed, so the six
cortical objects are independent and the only difference between arms is the E->E cut.
"""
...
def cell_tag(g, stimulus, seed, ee_cut=False):
    ...
    return f"{ARM_NAME[bool(ee_cut)]}_{arm_tag(g)}_{stimulus}_s{int(seed)}"


ARM_NAME = {False: "single_area", True: "ee_cut"}


def outputs_for(g, stimulus="on", seed=0, ee_cut=False):
    tag = cell_tag(g, stimulus, seed, ee_cut)
    return (ROOT / "results" / f"whole_system_oscillator_{tag}.json",
            ROOT / "results" / f"whole_system_oscillator_{tag}_y.npz")
...
def cut_local_ee(model, area, cls):
    """Set every within-area E->E weight to 0.0 and report what was realized.

    Returns the edge list unchanged in structure: edges are zeroed, never removed, so the
    two arms differ in weight alone and every other array keeps its shape and order.
    """
    edges = model.params["edge_list"]
    pre = np.asarray(edges.pre)
    post = np.asarray(edges.post)
    w = np.asarray(edges.weight).copy()
    same_area = area[pre] == area[post]
    ee = (cls[pre] == "E") & (cls[post] == "E")
    target = same_area & ee
    before = float(w[target].sum())
    w[target] = 0.0
    # EdgeList is a frozen dataclass: replace it rather than mutating it, so nothing else
    # holding the old object sees a silently changed weight array.
    model.params["edge_list"] = dataclasses.replace(edges, weight=w)
    after = np.asarray(model.params["edge_list"].weight)
    realized = {
        "configured": "every within-area E->E weight set to 0.0",
        "realized": {"n_edges_targeted": int(target.sum()),
                     "weight_sum_before": round(before, 6),
                     "weight_sum_after": round(float(after[target].sum()), 6),
                     "max_abs_after": float(np.abs(after[target]).max()) if target.any() else 0.0,
                     "n_edges_total": int(pre.size),
                     "n_edges_untouched": int((~target).sum()),
                     "untouched_weight_sum": round(float(after[~target].sum()), 6)},
        "executed": "the compiled kernel reads this edge list; no other array was modified",
        "effective": None,
    }
    if realized["realized"]["max_abs_after"] != 0.0:
        raise ValueError("EE_CUT did not zero every targeted edge")
...
def main(g_auth, stimulus='on', seed=0, ee_cut=False, out=None, out_y=None,
    ...
    default_out, default_out_y = outputs_for(g_auth, stimulus, seed, ee_cut)
    ...
    # The principal delta. Applied last, so every inherited value it does not target is
    # exactly what the intact arm compiles.
    ee_realization = None
    if ee_cut:
        model, ee_realization = cut_local_ee(model, area, cls)

    rec = {"spec": spec_name, "spec_commit": spec_commit, "g": g_auth,
           "arm": ARM_NAME[bool(ee_cut)], "g_tag": arm_tag(g_auth),
           "cell": cell_tag(g_auth, stimulus, seed, ee_cut),
           "ee_cut": bool(ee_cut), "ee_cut_realization": ee_realization,
