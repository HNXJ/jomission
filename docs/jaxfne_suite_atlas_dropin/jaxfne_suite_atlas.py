"""jaxfne_suite_atlas — an interactive Plotly atlas for any jaxfne circuit.

Drop-in, self-contained: one file, no imports beyond numpy and plotly. ``jaxfne`` itself is
imported lazily and only when ``signals`` is not supplied.

    from jaxfne_suite_atlas import build_suite_atlas
    build_suite_atlas(model, out_dir="atlas")          # simulates if signals is None
    build_suite_atlas(model, signals, out_dir="atlas", H_trace=H, w_trace=w)

Six panels, each its own standalone HTML with a caption and a provenance card, plus an
``index.html`` and a ``manifest.json``:

    network_3d     dark 3D explorer, areas along x, layers in depth, edges by projection type
    raster         spike raster ordered area -> layer -> class, over per-class population rate
    membrane       sampled V(t) per class with the pooled voltage occupancy
    field          LFP and CSD proxies as depth x time heatmaps, plus stacked LFP by depth
    connectivity   signed weight sums, area x area, layer x layer, class x class
    plasticity     H trajectories per class with 5-95th band, and |w|/|w0| quantiles

Everything is read from the model rather than configured:
...
Areas, layers and cell classes are whatever the model declares. Known names keep a fixed color
so figures stay recognisable across circuits; unknown names draw from a palette in a stable
order, so a circuit with areas this file has never seen still renders with a consistent legend.
...
# ======================================================================================
# theme — the visual standard, inlined so this file has no dependency beyond numpy+plotly
# ======================================================================================

AREA_COLORS = {"V1": "#38bdf8", "V4": "#4ade80", "FEF": "#fb923c", "PFC": "#c084fc"}
CLASS_COLORS = {"E": "#06b6d4", "PV": "#f43f5e", "SST": "#eab308", "VIP": "#a855f7"}
PROJ_COLORS = {"recurrent": "#64748b", "FF": "#22c55e", "FB": "#f97316"}

_FONT = "Inter, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif"
_AXIS = dict(gridcolor="#21262d", zerolinecolor="#30363d", tickcolor="#8b949e",
             linecolor="#30363d")

DARK_TEMPLATE = dict(layout=go.Layout(
    paper_bgcolor="#0d1117", plot_bgcolor="#161b22",
    font=dict(color="#c9d1d9", family=_FONT, size=12),
...
def apply_dark_theme(fig: "go.Figure", title: str, subtitle: str = "") -> "go.Figure":
    """Consistent dark styling, title and fonts."""
    full = f"<b>{title}</b>"
    if subtitle:
        full += (f"<br><span style='font-size:12px; color:#8b949e; font-weight:normal'>"
                 f"{subtitle}</span>")
    fig.update_layout(template=DARK_TEMPLATE,
                      title=dict(text=full, x=0.04, y=0.96, xanchor="left", yanchor="top"))
    return fig


def wrap_figure_with_provenance_html(fig: "go.Figure", caption: str, provenance: dict,
                                     evidence_level: str = "OBSERVED") -> str:
    """Standalone HTML: the figure, its caption, and a provenance card stating where it came from."""
    fig_html = fig.to_html(include_plotlyjs="cdn", full_html=False)
    badge = {"OBSERVED": "#238636", "DERIVED": "#1f6feb"}.get(evidence_level, "#d29922")
    rows = "".join(
        f"<tr><td style='color:#8b949e; padding:3px 12px 3px 0;'><b>{k}</b></td>"
        f"<td style='color:#c9d1d9; font-family:monospace;'>{v}</td></tr>"
        for k, v in provenance.items())
    ttl = fig.layout.title.text if fig.layout.title else "jaxfne suite atlas"
    return f"""<!DOCTYPE html>
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>{ttl}</title>
<style>
  body {{ background:#0d1117; color:#c9d1d9; font-family:{_FONT};
         margin:0; padding:24px; display:flex; flex-direction:column; align-items:center; }}
  .container {{ width:100%; max-width:1280px; }}
  .caption-box {{ background:#161b22; border:1px solid #30363d; border-radius:6px;
                  padding:16px 20px; margin-top:16px; font-size:13px; line-height:1.6; }}
  .provenance-card {{ background:#161b22; border:1px solid #30363d; border-radius:6px;
                      padding:14px 20px; margin-top:12px; font-size:12px; }}
  .badge {{ display:inline-block; padding:2px 8px; border-radius:12px; font-size:11px;
            font-weight:600; color:#fff; background:{badge}; margin-bottom:8px; }}
  ...
  {fig_html}
  <div class="caption-box"><span class="badge">{evidence_level}</span><div>{caption}</div></div>
  <div class="provenance-card"><b style="color:#f0f6fc;">Provenance &amp; Execution Audit</b>
