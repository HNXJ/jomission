"""WS-AUTH arm driver: the same whole-system diagnostic under EQUAL_G long-range coupling.

    python scripts/whole_system_authority_arm.py --g 0.2

Forked from scripts/whole_system_diagnostic_3.py rather than editing it: that file is a sealed
artifact of WS-DIAG-3. Every change relative to it is additive and listed here:

    EQUAL_G realization   after interface enforcement and before compilation, every long-range
                          cortical weight is replaced by w_p,i = g * K_local,i / N_afferents,p,i.
                          Connectivity, delay, tau, receptor, local weights, retinal edges and
                          drive are untouched, and the receipt asserts that.
    pre-execution gate    max_i |K_long,i/K_local,i - g| < eps is checked BEFORE any simulation
                          and raises rather than running an arm whose realization is wrong.
    arm identity          --g selects the arm; outputs are named by it so the four arms of the
                          bracket are separate artifacts rather than one overwritten file.
    elapsed               wall time is recorded, which the parent driver did not do.

The kernel is baseline: HDP is OFF for this bracket, matching WS-DIAG-2, so that long-range
authority is the single principal delta. Gates are evaluated in the sealed order and the arm
stops at the first load-bearing failure.
"""
...
configure()
def arm_tag(g):
    """Stable filename tag for an arm: 0.05 -> g005, 0.5 -> g050."""
...
def main(g, out=None, out_y=None, spec_name=DEFAULT_SPEC, spec_commit=None):
    ...
    default_out, default_out_y = outputs_for(g)
    ...
    if not authority["acceptance"]["pass"]:
        raise ValueError(f"EQUAL_G realization failed its own acceptance at g={g}: "
                         f"max|K_long/K_local - g| = {authority['acceptance']['max_abs_error']}")

    rec = {"spec": spec_name, "spec_commit": spec_commit, "g": g, "arm": arm_tag(g),
    ...
    print("arm %s (g=%g) | %s | %s | %.0fs"
          % (rec["arm"], g, rec["verdict"], rec["fail_boundary"], rec["elapsed_s"]))
