"""SCI-EI-REGIME-1 arm driver: why are PV and VIP near-silent and SST hyperactive?

Forked from scripts/whole_system_oscillator_arm.py, a sealed artifact of
SCI-WS-OSC-1, rather than edited in place. The sealed source is unchanged.

Additive changes, no intervention (diagnosis only):
  retention     per-area-per-class spike counts at full 1 ms resolution instead
                of the 10 ms decimated group means; the 1 ms binning sums exact
                spike counts, so the subsampling caveat is removed, not worked around
  record        an ei_regime block: per-class rates at full resolution over the full
                run and per phase, the B1 comparison against the ratified targets,
                and the four predeclared candidate checks with a first-passing
                attribution. No parameter is changed and no rate is calibrated.

Construction, regime, tonic, dt, seed, duration, kernel, delay, stimulus and
the run loop are the intact SINGLE_AREA arm's, untouched.
"""
...
def outputs_for(g, stimulus="on", seed=0):
    ...
    return (
        ROOT / "results" / f"whole_system_ei_regime_{tag}.json",
        ROOT / "results" / f"whole_system_ei_regime_{tag}_y.npz",
    )
