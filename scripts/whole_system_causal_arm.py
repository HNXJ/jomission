"""WS-PROP causal arm driver: one cell of the ON/OFF by coupling design.

    python scripts/whole_system_causal_arm.py --g 0.0 --stimulus off --seed 0

Forked from scripts/whole_system_authority_arm_v2.py rather than editing it: that file is a
sealed artifact of WS-AUTH-2 (blob fe8d0140, sealed_in ff68971). Every change is listed here.

WS-AUTH-2 showed the within-run window difference cannot identify transmission: at g = 0, with
no current able to reach them, FEF and PFC still reported +0.80 and +0.84 Hz. The repaired
estimator needs four matched runs,

    R_A(g) = [r_A_on(g) - r_A_off(g)] - [r_A_on(0) - r_A_off(0)]

and this driver produces one of them. Additive changes:

    stimulus on/off   --stimulus off lights no retinal unit, so the retinal schedule is zero
                      everywhere for the whole run while the interface, the tonic drive and
                      every other input stay exactly as they are. ON and OFF differ in the spot
                      and in nothing else.
    seed              --seed sets the continuation PRNG key, which v2 pinned at 0. ON and OFF of
                      one pair MUST share a seed, so their difference is the stimulus and not
                      the noise stream. Distinct seeds exist to build a held-out null replicate,
                      whose spread is the only defensible tolerance for R.
    legacy field      the old within-run propagation gate still runs and its numbers are
                      unchanged, so this arm stays comparable to every earlier one. It now
                      carries a qualification naming it non-identifying under this architecture.
                      Historical results are NOT rewritten.
    naming            outputs are tagged by coupling, stimulus and seed, so no cell of the
                      design can overwrite another.

Everything else is inherited unchanged: the EQUAL_G realization and its pre-execution
acceptance, the kernel, the gates, the sealed order and the series-to-npz storage.
"""
...
def cell_tag(g, stimulus, seed):
    """Name one cell of the ON/OFF by coupling design: g000_off_s0, g500_on_s0, ..."""
    if stimulus not in ("on", "off"):
        raise ValueError(f"stimulus must be 'on' or 'off', not {stimulus!r}")
    return f"{arm_tag(g)}_{stimulus}_s{int(seed)}"


def outputs_for(g, stimulus="on", seed=0):
    tag = cell_tag(g, stimulus, seed)
    return (ROOT / "results" / f"whole_system_causal_{tag}.json",
            ROOT / "results" / f"whole_system_causal_{tag}_y.npz")
...
def main(g_auth, stimulus='on', seed=0, out=None, out_y=None, spec_name=DEFAULT_SPEC,
         spec_commit=None):
    ...
    rec = {"spec": spec_name, "spec_commit": spec_commit, "g": g_auth,
           "arm": arm_tag(g_auth), "cell": cell_tag(g_auth, stimulus, seed),
