"""SCI-EI-INTERACTION-1 cell driver: weight repair by adaptation repair factorial.
...
Additive changes, one cell per invocation:
  repairs       (1) E->PV and E->VIP mean weights to parity with realized E->E,
                multiplicative per edge, factors measured from the cell's own
                realized arrays and asserted against the EI diagnosis receipt;
                (2) SST adaptation a 0.05 to canonical LTS 0.02, asserted built
                value first. Applied after EQUAL_G and before compilation.
  record        the ei_regime block is kept per cell (rates, candidate readings),
                plus per-cell viability against the ratified B1 bands, synchrony
                (E-rate CV final 1000 ms) and the 94-95 ms mode by the sealed
                periodicity estimator. No verdict here: classification over the
                four cells lives in scripts/whole_system_ei_interaction_compare.py.

Construction, regime, tonic, dt, seed, duration, kernel, delay and stimulus are
the intact SINGLE_AREA arm's, untouched.
"""
...
def cell_tag(g, stimulus, seed, cell="baseline"):
    """Name one cell: the arm, the stimulus, the seed and the factorial cell."""
    if stimulus not in ("on", "off"):
        raise ValueError(f"stimulus must be 'on' or 'off', not {stimulus!r}")
    if cell not in CELLS:
        raise ValueError(f"cell must be one of {CELLS}, not {cell!r}")
    return f"intact_{arm_tag(g)}_{stimulus}_s{int(seed)}_{cell}"


def outputs_for(g, stimulus="on", seed=0, cell="baseline"):
    tag = cell_tag(g, stimulus, seed, cell)
    return (
        ROOT / "results" / f"whole_system_ei_interaction_{tag}.json",
        ROOT / "results" / f"whole_system_ei_interaction_{tag}_y.npz",
    )
...
def apply_repairs(model, area, layer, cls, cell, expected):
    """Apply this cell's predeclared repairs after EQUAL_G, before compilation.

    expected carries the EI diagnosis receipt values the construction must
    reproduce first (construction-identity gate): realized E->E/PV/VIP means
    and the built SST adaptation value. Any mismatch raises instead of
    repairing different values.
    """
    import dataclasses

    receipt = {"cell": cell, "repairs": []}
    if cell not in CELLS:
    ...
    for name, got, want in (
        ("E->E", wee, expected["wee"]),
        ("E->PV", wepv, expected["wepv"]),
        ("E->VIP", wevip, expected["wevip"]),
    ):
        if abs(got - want) > 1e-6:
            raise ValueError(f"construction identity failed: {name} {got} != receipt {want}")
    if cell in ("w_only", "both"):
        f_pv, f_vip = wee / wepv, wee / wevip
        w2 = w.copy()
        w2[(cls[pre] == "E") & (cls[post] == "PV")] *= f_pv
        w2[(cls[pre] == "E") & (cls[post] == "VIP")] *= f_vip
        model.params["edge_list"] = dataclasses.replace(el, weight=w2)
        receipt["repairs"].append(
            {
                "kind": "weight_parity",
                "factor_E_to_PV": f_pv,
                "factor_E_to_VIP": f_vip,
                "authority": "parity with realized E->E mean (internal reference, never rate-tuned)",
            }
        )
    em = model.params["emitter"]
    sst_idx = np.flatnonzero(cls == "SST")
    a_vals = sorted(set(np.atleast_1d(np.asarray(em.a, dtype=float))[sst_idx].tolist()))
    if a_vals != [expected["sst_a"]]:
        raise ValueError(
            f"construction identity failed: SST a {a_vals} != receipt {[expected['sst_a']]}"
        )
    if cell in ("a_only", "both"):
        a = np.asarray(em.a, dtype=float).copy()
        a[sst_idx] = 0.02
        model.params["emitter"] = dataclasses.replace(em, a=jnp.asarray(a, dtype=em.a.dtype))
        receipt["repairs"].append(
            {
                "kind": "sst_canonical_LTS",
                "a_before": expected["sst_a"],
                "a_after": 0.02,
                "authority": "Izhikevich2003 LTS canonical (b stays 0.25, c and d untouched)",
            }
        )
...
def main(
    ...
    default_out, default_out_y = outputs_for(g_auth, stimulus, seed, cell)
    ...
    # The factorial cell. Repairs apply after EQUAL_G and before compilation, so the
    # kernel only ever sees the repaired construction. Baseline applies none.
    model, repair_receipt = apply_repairs(
        model, area, layer, cls, cell, SPEC["repairs"]["expected"]
    )
    ...
    rec = {
        ...
        "cell_tag": cell_tag(g_auth, stimulus, seed, cell),
        "stimulus": stimulus,
        "seed": int(seed),
        "pairing": (
    ...
    rec["candidate_readings"] = {k: bool(s) for k, s in order}

    viable = bool(
        (12.0 <= rates["PV"]["full"] <= 25.0)
        and (8.0 <= rates["VIP"]["full"] <= 15.0)
        and (6.0 <= rates["SST"]["full"] <= 12.0)
        and (rates["E"]["full"] < rates["PV"]["full"])
    )
    rec["viability"] = {
        "bands": "ratified B1 globals: PV [12,25], VIP [8,15], SST [6,12], plus E < PV",
        "viable": viable,
        "note": "classification, not a tuning target; the verdict over cells lives in the compare output",
    }
    n_e_total = int(sum(members[g].size for g in e_groups))
    pooled_e = (
        sum(
            e_rate[a] * sum(members[g].size for g in e_groups if g.split(".")[0] == a)
            for a in areas
        )
        / n_e_total
    )
    from jomission.harness import oscillation as _osc

    mode = _osc.measure(
        window(pooled_e, TOTAL_MS - 1000.0, TOTAL_MS),
        dt_ms=1.0,
        n_cells=n_e_total,
        max_lag_ms=500.0,
        n_surrogate=200,
    ...
    rec["mode"] = {
        ...
        "window_ms": [TOTAL_MS - 1000.0, TOTAL_MS],
        "present": bool(mode["periodic_mode_present"]),
        "period_ms": mode["period_ms"],
    }
    rec["synchrony"] = {
        "e_cv_final_1000ms": {a: gates["synchrony"]["cv_final_1000ms"][a] for a in areas},
        "note": "same computation as the run-health synchrony gate; reported, not gating",
    }
    ...
    if first is not None:
        rec["verdict"] = "EI_CELL_EXECUTION_FAIL"
        rec["fail_boundary"] = f"run-health gate failed: {first}; rates below are degenerate"
    else:
        rec["verdict"] = "EI_CELL_COMPLETE"
        rec["fail_boundary"] = (
            "cell completed; classification over cells lives in the compare output"
        )
    np.savez_compressed(
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--cell",
        choices=("baseline", "w_only", "a_only", "both"),
        default="baseline",
        help="factorial cell: exact baseline rebuild or a predeclared repair",
    )
