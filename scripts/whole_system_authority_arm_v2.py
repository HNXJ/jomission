"""WS-AUTH arm driver v2: the same arm, with three repairs the sealed v1 could not receive.

    python scripts/whole_system_authority_arm_v2.py --g 0.0

Forked from scripts/whole_system_authority_arm.py rather than editing it: that file is a sealed
artifact of WS-AUTH-1 (blob 6bfb97dd, sealed_in 74ee2cd) and records what those four arms
executed. Every change relative to it is listed here, and none of them alters a computed value,
a gate, a tolerance or a default:

    shadowed parameter   v1 named its authority parameter g and then rebound g as a loop
                         variable over group names, so its trailing summary print applied %g to
                         a string and every arm exited nonzero AFTER writing a correct result.
                         The parameter is renamed g_auth, which cannot be shadowed by those
                         loops. Queued as HD-ARM-DRIVER-SHADOWED-G.
    arm_tag docstring    v1's docstring claimed 0.05 -> g005 and 0.5 -> g050. The function
                         produces g050 and g500. The mapping is unchanged; the docstring now
                         states it correctly.
    series storage       reviewer 2026-09-19: the per-group series moves out of the result JSON
                         and into the .npz that is already written and already gitignored. v1's
                         result files were 16.5 MB each, of which the series was essentially all.
                         The JSON keeps gates, verdict, acceptance and summary statistics, and
                         records where the series went. The numbers are identical; only their
                         container changes.

Everything else is inherited unchanged, including the EQUAL_G realization, its pre-execution
acceptance, the kernel, the gates and the sealed order.
"""
...
def outputs_for(g):
    return (ROOT / "results" / f"whole_system_authority_{arm_tag(g)}.json",
            ROOT / "results" / f"whole_system_authority_{arm_tag(g)}_y.npz")
...
def main(g_auth, out=None, out_y=None, spec_name=DEFAULT_SPEC, spec_commit=None):
    ...
    default_out, default_out_y = outputs_for(g_auth)
    ...
    print("arm %s (g=%g) | %s | %s | %.0fs"
          % (rec["arm"], g_auth, rec["verdict"], rec["fail_boundary"], rec["elapsed_s"]))
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    configure(a.spec)
    main(a.g, Path(a.out) if a.out else None, Path(a.out_y) if a.out_y else None,
         a.spec, a.spec_commit)   # a.g is the authority; main names it g_auth
