...
def test_drive_additivity_mean_controlled_shot_passes_and_tonic_copy_fails():
    spec = importlib.util.spec_from_file_location(
        "_v21b", ROOT / "tests" / "test_v21b_operation.py"
    )
    ...
    sched = next(chunks)
    out = check_additive_schedule(
        drive, sched, cls, expected_schedule_mean={c: 0.0 for c in COUNTS}
    )
    ...
    with pytest.raises(DriveAdditivityError, match="expected"):
        check_additive_schedule(
            drive, sched + 0.5 * drive, cls, expected_schedule_mean={c: 0.0 for c in COUNTS}
        )
...
def test_sealed_artifacts_unchanged():
    ...
    old_handoff = subprocess.run(
        ["git", "rev-parse", "a488348:docs/HANDOFF_CURRENT.md"],
        cwd=ROOT,
        capture_output=True,
        text=True,
    ).stdout.strip()
    edited = {
        "artifacts": [
            {"path": "docs/HANDOFF_CURRENT.md", "blob": old_handoff, "sealed_in": "a488348"}
        ]
    }
...
def test_lineage_validator_rejects_defects():
    ...
    rec["preexecution_spec"]["commit"], rec["commit"] = (
        rec["commit"],
        rec["preexecution_spec"]["commit"],
    )
    ...
    rec["observations"].append(
        {
            ...
            "receipt": "results/v21b_lineage.json",
            "kind": "transient",
            "estimator": {},
        }
    )
    ...
    for needle in (
        "labelled INFERRED",
        "without resolvable receipt",
        "labelled OBSERVED",
        "estimator.bin_width_ms",
        "missing stage realized",
    ):
        assert any(needle in e for e in errs), (needle, errs)
    rec3 = copy.deepcopy(rec)
    rec3.pop("qualification")
    assert any(
        "without configured/realized/executed/effective" in e for e in v.validate_lineage(rec3)
    )
...
def test_lineage_observation_checks_value_match():
    rec = next(
        r
        for r in v.load("manifests/lineage_registry.json")["lineages"]
...
def test_results_coverage_registered():
    ...
    assert errs == [
        "unregistered result results/planted_unregistered.json "
...
PASS_ASSERT_ALLOWLIST = {
    ...
    "tests/test_ei_recruitment.py": "EQUAL_G realization acceptance is a construction precondition for the read-only measurement, not a scientific verdict; the lineage verdict is UNRESOLVED either way",
}
...
def test_scientific_verdicts_do_not_fail_pytest():
    ...
    assert pat.search('    assert res["pass"]') and pat.search(
        'assert verdict == "V2_LOCAL_OPERATION_PASS"'
    )
...
def test_new_run_continuation_call_sites_use_drive_guard():
    ...
    hist = {
        e["path"]: e["blob"]
        for e in v.load("manifests/harness/historical_callsites.json")["run_continuation"]
    }
    ...
    assert offenders == [], (
        f"new or edited run_continuation call sites must use jomission.harness.drive: {offenders}"
    )
