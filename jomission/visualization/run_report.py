...
# Transfer Function stage semantics per jomission/visualization/transfer.py
# ---------------------------------------------------------------------------
_TAB_SEMANTIC_CLASS: dict[str, str] = {
    ...
    "connectivity": "STATE",
    "diagnostics": "DERIVED",
    "transfer_function": "STATE",
    "transfer_summary": "STATE",
    "provenance": "READOUT",
}
...
_TAB_DERIVED_FROM: dict[str, str] = {
    ...
    "transfer_function": "V1 L4E spikes → delay/history (B_t) → syn_state → edge_current (w·syn_state) → L2/3E Vm → L2/3E spikes (per jomission/visualization/transfer.py, builder.py:62,90, jaxfne/emitters.py:545, _model_simulate.py:280)",
    "transfer_summary": "Transfer chain summary latencies/effect sizes across 6 stages (DERIVED_FROM per-stage STA)",
...
_FIXED_TABS: list[tuple[str, str]] = [
    ...
    ("Transfer Function", "Mechanistic oscilloscope: V1 L4E spikes → delay/history (B_t) → L4E→L23E syn_state → L4E→L23E current → L2/3E Vm → L2/3E spikes — each arrow STA, A/B diff, effect size, latency, units, provenance, measured/derived/proxy/unavailable (NOT OBSERVABLE when jaxfne/_model_simulate.py:280 HDP+delay blocks edge_current)"),
    ("Transfer Summary", "Transfer chain summary latencies/effect sizes across 6 stages"),
...
def _build_provenance_table(simulation_result: dict, diag: dict, out_dir: pathlib.Path) -> list[dict]:
    ...
    rows = [
        ...
        {"claim": "Transfer V1 L4E spikes — STATE (STA)", "estimator": "PSTH V1 L4 E spikes aligned to stimulus onset (transfer.py:sta)", "estimator_version": "transfer.py:sta v0", "array": "spikes [T,N] filtered V1 L4 E (9 units)", "hash": h_spike, "source": "jtfne.simulate → Signals.spikes same array (builder.py:62, jaxfne/emitters.py:211)", "semantic_class": "STATE", "units": "Hz", "derived_from": "X_t spikes", "proxy_status": False},
        {"claim": "Transfer delay/history — STATE history", "estimator": "delay_steps distribution + ContinuationState.delay_state shape (jaxfne/emitters.py:545, _pipeline.py:72)", "estimator_version": "transfer.py:delay v0", "array": "delay_state [D_max,N] + edge_list.delay_steps [n_edges] (builder.py:90 2/8/12 ms →20/80/120)", "hash": "delay_steps hash via edge_list", "source": "builder.py:807 _apply_laminar_delays → emitters.py:545 → _pipeline.py:72 ContinuationState.delay_state", "semantic_class": "STATE", "units": "steps/ms (delay) + bool buffer", "derived_from": "spikes delayed via B_t", "proxy_status": False},
        {"claim": "Transfer L4E→L23E syn_state — STATE/PROXY", "estimator": "syn_state [T,n_edges] if recorded else proxy filtered spikes tau 2 ms (emitters.py:457)", "estimator_version": "transfer.py:syn_proxy v0", "array": "syn_state [T,n_edges] or spikes [T,N] convolved proxy", "hash": h_spike+"+proxy", "source": "jaxfne/_pipeline.py:48 syn_state same-owner when available else spikes proxy", "semantic_class": "STATE", "units": "dimensionless gating a.u.", "derived_from": "spikes filtered", "proxy_status": True},
        {"claim": "Transfer L4E→L23E current — READOUT (NOT OBSERVABLE when blocked by _model_simulate.py:280)", "estimator": "edge_current_trace w·syn_state via emitters.py:2846 record_edge_current (machine check delay>0 & HDP)", "estimator_version": "observables.py:35 partition_currents_by_motif", "array": "edge_current_trace [T,n_edges] optional (blocked when delay_steps [20,80,120] + HDP per _model_simulate.py:280, _pipeline.py:395 only hdp)", "hash": "blocked machine-derived", "source": "model.last_hdp_diagnostics() → observables.py:35 (requires record_edge_current; blocked by _model_simulate.py:280 guard)", "semantic_class": "READOUT", "units": "a.u. native current (w·syn_state)", "derived_from": "w·syn_state", "proxy_status": False},
        {"claim": "Transfer L2/3E Vm — STATE", "estimator": "STA V1 L2/3 E V_m [T,N] aligned (transfer.py)", "estimator_version": "transfer.py:vm_sta v0", "array": "V_m [T,N] mV filtered V1 L2/3 E (30 units) same array", "hash": h_vm, "source": "jtfne.simulate → Signals.V_m (observable_basis jomission/visualization/model_summary.py:822)", "semantic_class": "STATE", "units": "mV", "derived_from": "X_t V_m", "proxy_status": False},
        {"claim": "Transfer L2/3E spikes — STATE", "estimator": "PSTH V1 L2/3 E spikes aligned", "estimator_version": "transfer.py:spike_sta v0", "array": "spikes [T,N] filtered V1 L2/3 E (30 units)", "hash": h_spike, "source": "jtfne.simulate → Signals.spikes same array", "semantic_class": "STATE", "units": "Hz", "derived_from": "X_t spikes", "proxy_status": False},
    ]
    return rows
...
def _render_html(
    ...
    for tid in _TAB_IDS:
        ...
        elif tid == "transfer_function":
            src_arr, ahash, est, est_ver, units, proxy = "spikes+V_m+EdgeList+delay_state", h_spike_tmp, "STA per stage (V1 L4E→L23E) + edge_current blocked_machine_check", "transfer.py:sta v0 (observables.py:35, _model_simulate.py:280, emitters.py:545, builder.py:62,90)", "mixed (Hz/mV/a.u./steps)", True
            n_tot, n_rend, filt = n_neurons * n_steps, n_neurons, "V1 L4 E 9, L2/3 E 30, L4→L23 edges ~76, delay D_max 120; STA -50..300 ms, Cohen d, latency; machine blocked check delay>0 & HDP"
        elif tid == "transfer_summary":
            src_arr, ahash, est, est_ver, units, proxy = "transfer chain summary", h_spike_tmp, "latency/effect size across 6 stages aggregated", "transfer.py:summary v0", "ms / Cohen d dimensionless", False
            n_tot, n_rend, filt = 6, 6, "6 stages V1 L4E→L23E chain summary"
        elif tid == "overview":
    ...
    for tab_id, (tab_name, tab_desc) in zip(_TAB_IDS, _FIXED_TABS):
        ...
        if tab_id == "transfer_function" and tab_id in figs and isinstance(figs[tab_id], dict) and figs[tab_id]:
            # Transfer Function tab: 6 panels composite (V1 L4E spikes → ... → L2/3E spikes)
            # figs[tab_id] is dict stage_id -> go.Figure (6 panels), each with STA, A/B diff, d, latency, NOT OBSERVABLE banner machine-derived
            stage_dict: dict = figs[tab_id]  # type: ignore
            # need per-stage provenance from overview transfer_provenance if available
            t_prov = overview.get("transfer_provenance", {}) if isinstance(overview, dict) else {}
            frags = []
            for stage_id, stage_fig in stage_dict.items():
                if stage_fig is None:
                    frags.append(f'<div class="fig" style="border:1px dashed #d62728; padding:8px; margin:8px 0;"><em>Panel {stage_id} unavailable</em></div>')
                    continue
                try:
                    sub_frag = pio.to_html(stage_fig, include_plotlyjs=False, full_html=False, div_id=f"fig-{tab_id}-{stage_id}") if pio else f"<div>Fig {stage_id}</div>"
                except Exception:
                    sub_frag = f"<div>Stage {stage_id} render error</div>"
                # per-stage provenance details
                st_prov = t_prov.get(stage_id, {}) if isinstance(t_prov, dict) else {}
                status = str(st_prov.get("status", ""))
                badge_stage = f'<span class="badge sem-{str(st_prov.get("semantic_class","")).lower()}">semantic_class {st_prov.get("semantic_class","")}</span> <span class="badge proxy-{str(st_prov.get("proxy_status","")).lower()}">proxy_status {st_prov.get("proxy_status","")}</span> <span class="badge" style="background:#d62728;color:#fff;" >status {status}</span>'
                frags.append(f'<div class="fig" style="margin:12px 0; border:1px solid #ddd; border-radius:8px; padding:8px;"><h3 style="margin:4px 0;font-size:13px;">{stage_id} — {st_prov.get("title","")} — units <code>{st_prov.get("units","")}</code></h3><p>{badge_stage}</p><div id="fig-{tab_id}-{stage_id}">{sub_frag}</div><details class="prov-footer"><summary>Per-stage provenance {stage_id}</summary><pre>{json.dumps(st_prov, indent=2, sort_keys=True)}</pre></details><p class="prov-meta">figure_id <code>{st_prov.get("figure_id","")}</code> array <code>{st_prov.get("array","")}</code> hash <code>{st_prov.get("array_hash","")}</code> source <code>{st_prov.get("source","")}</code></p></div>')
            frag = "\n".join(frags) if frags else "<p><em>No transfer panels generated.</em></p>"
        ...
        elif tab_id in figs and figs[tab_id] is not None:
            fig = figs[tab_id]
            try:
...
def run_report(simulation_result: dict) -> dict:
    ...
    # Transfer Function + Summary (mechanistic oscilloscope)
    try:
        from jomission.visualization.transfer import transfer_figures, summary_panel_fig  # type: ignore
        t_figs, t_prov, t_summary = transfer_figures(simulation_result)
        # Map 6 stage figs to transfer_function composite (use first stage as tab representative + store all for composite rendering)
        # For HTML tab, we combine 6 panels into one composite via subplots wrapper; but run_report's html expects one fig per tab.
        # Instead, build a composite Transfer Function figure stacking the 6 stages as separate html fragments handled in _render_html.
        # Store t_figs as special entry "transfer_function" dict for later rendering.
        figs["transfer_function"] = t_figs  # dict stage_id -> fig (handled specially in _render_html)
        figs["transfer_summary"] = summary_panel_fig(t_prov, t_summary)
        overview["transfer_summary"] = t_summary
        overview["transfer_provenance"] = t_prov
        # Save transfer-specific arrays for audit
        try:
            (arrays_dir / "transfer_summary.json").write_text(json.dumps(t_summary, indent=2, sort_keys=True))
            (arrays_dir / "transfer_provenance.json").write_text(json.dumps(t_prov, indent=2, sort_keys=True))
        except Exception:
            pass
    except Exception as e:
        figs["transfer_function"] = None
        figs["transfer_summary"] = None
        overview["transfer_error"] = str(e)
    ...
    for tab_id, fig in figs.items():
        ...
        try:
            ...
            if isinstance(fig, dict):
                # Transfer function composite: dict stage_id -> fig
                for stage_id, stage_fig in fig.items():
                    if stage_fig is None:
                        continue
                    try:
                        stage_fig.write_html(str(figs_dir / f"{tab_id.replace('/','_').replace(' ', '_')}_{stage_id}.html"), include_plotlyjs="cdn", full_html=True)
                    except Exception:
                        pass
                continue
...
def generate_example_C021_transfer(
    *,
    duration_ms: float = 2000.0,
    seed: int = 0,
    dt_ms: float = 0.1,
    n_per_area: int = 100,
    out_dir: str = "results/visualization/example_run_C021_transfer",
) -> dict:
    """Generate C021 transfer example (a51711101576c7c3 RF-distinct, interleaved tiling, PV gain 0.7).

    C021 parent 4574a15519ee8c33 → a51711101576c7c3 (rf_hash 18c2f0f55229c307) per
    scratch/gen2_C021_receipt.md:4. Uses build_jomission_model(seed=0) for
    simulation (so delays [20,80,120] via builder.py:90, emitters.py:545 are
    preserved) while config_hash is overridden to a5171110… RF-distinct for
    provenance (RF only affects drive, not weights/topology). Silent schedule
    (spontaneous) so Transfer Function shows spontaneous baseline per stage with
    machine-derived NOT OBSERVABLE for L4E→L23E current when
    jaxfne/_model_simulate.py:280 blocks (delay_steps [20,80,120] + HDP). That
    banner is machine-derived via jomission/visualization/transfer.py:
    _is_edge_current_blocked (checks edge_list.delay_steps>0 and enable_hdp,
    not manually typed).

    Writes via run_report (which now includes Transfer Function + Summary tabs
    with 6 panels) to out_dir and returns its result.
    """
    import jaxfne as jtfne  # type: ignore
    from jaxfne import Simulation, RuntimeConfig  # type: ignore
    import jaxfne.hdp_network as hdp  # type: ignore
    from jaxfne.io import config_hash  # type: ignore
    from jomission.network.builder import build_jomission_model, build_jomission_network  # type: ignore

    # Build base model for simulation (preserves delays [20,80,120])
    base_model = build_jomission_model(n_per_area=n_per_area, seed=seed, dt_ms=dt_ms)
    # Compute RF-distinct config_hash without losing delays: overlay RF metadata onto base cfg
    ch = "unknown"
    try:
        from jomission.network.rf import RFConfig, apply_rf_to_configuration  # type: ignore
        rf_cfg = RFConfig(seed=seed)  # interleaved True, visual_pv_gain 0.7, v0.2.0 → rf_hash 18c2f0f55229c307
        cfg_rf = apply_rf_to_configuration(build_jomission_network(n_per_area=n_per_area, seed=seed, dt_ms=dt_ms), rf_cfg)
        ch = config_hash(cfg_rf)  # expected a51711101576c7c3
    except Exception:
        try:
            ch = config_hash(base_model.cfg)
        except Exception:
            ch = "unknown"
    model = base_model
    ...
    try:
        ...
        try:
            n_neurons = int(len(model.neuron_table()))
        except Exception:
            n_neurons = 400
    sched = StimulusSchedule(events=(), n_neurons=n_neurons)
    ...
    sim_result = {
        ...
        "dt_ms": dt_ms,
        "duration_ms": float(duration_ms),
        "run_id": "example_run_C021_transfer",
        "results_dir": out_dir,
        "paradigm": sched,
        "hdp_used": hdp_used,
        "hdp_error": hdp_error,
        "phases": ["spontaneous"],
        "completion": {"all": True, "note": "C021 transfer spontaneous baseline; Transfer Function shows 6 stages V1 L4E→L23E chain, machine NOT OBSERVABLE for current when blocked"},
    }
    return run_report(sim_result)


if __name__ == "__main__":
    # Generate example when run as script — C021 transfer (new) then C019
    try:
        res = generate_example_C021_transfer()
        print(json.dumps({k: res[k] for k in ["run_id", "out_dir", "report_html", "wall_time_s"]}, indent=2))
    except Exception as e:
        print(f"C021 transfer generation failed: {e}")
