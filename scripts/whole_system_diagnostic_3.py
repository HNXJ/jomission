"""WS-DIAG-3 driver: the same whole-system diagnostic with the HDP kernel enabled.

    python scripts/whole_system_diagnostic_3.py --spec results/whole_system_diagnostic_3_spec.json

Forked from scripts/whole_system_diagnostic.py rather than editing it: that file is a sealed
artifact of WS-DIAG-2 (blob 208cae2e, sealed_in 696a597) and records what WS-DIAG-2 actually
executed. Editing it in place broke the seal, which is repaired by this fork. Every change
relative to the sealed driver is additive and listed here:

    output arity      the hdp kernel silently ignores record_current_trace and record_u_trace,
                      returning 4 outputs (v, spikes, sources, H) where baseline returns 6.
                      Recording adapts, and the summary omits the columns the kernel did not
                      produce instead of fabricating them.
    registered rule   a spec naming hdp_rule_module has that module's ensure_registered() called
                      before compile_step_fn, and its rule params are passed through.
    tonic income      a registered rule is not given params.drive, so the per-neuron tonic is
                      injected via hdp_rule_params when the spec asks for it. Without this H's
                      income is the synaptic term alone.
...
DEFAULT_SPEC = "results/whole_system_diagnostic_3_spec.json"
...
configure()
OUT = ROOT / "results" / "whole_system_diagnostic_3.json"
OUT_Y = ROOT / "results" / "whole_system_diagnostic_3_y.npz"
